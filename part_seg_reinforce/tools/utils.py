...
def clustering_acc_new(y_true, y_pred, num_points):
    ...
    def discretize(arr):
        ...
        label = 0
        for x in arr:
            if x in from_to.keys():
                continue
            from_to.update({x: label})
            assert label not in to_from.keys()
            to_from.update({label: x})
            label += 1
        return from_to, to_from

    matched_pred = []
    for point_id in range(0, y_pred.shape[0], num_points):
        cur_true = y_true[point_id:point_id + num_points].copy()
        true_from_to, true_to_from = discretize(cur_true)
        for i in range(num_points):
            cur_true[i] = true_from_to[cur_true[i]]
        num_gt = max(true_to_from.keys()) + 1
        cur_pred = y_pred[point_id:point_id + num_points].copy()
        pred_from_to, pred_to_from = discretize(cur_pred)
        for i in range(num_points):
            cur_pred[i] = pred_from_to[cur_pred[i]]
        num_pred = max(pred_to_from.keys()) + 1
        cost = np.zeros((num_pred, num_gt), dtype=np.int64)
    ...
    return {'matched_pred': matched_pred, 'accuracy': acc}


def clustering_acc_greedy(y_true, y_pred, num_points):
    from sklearn.metrics import accuracy_score
    matched_pred = []
    for point_id in range(0, y_pred.shape[0], num_points):
        cur_true = y_true[point_id:point_id + num_points]
        cur_pred = y_pred[point_id:point_id + num_points]
        unique_labels = set(cur_true)
        new_pred = -cur_pred
        for gt_label in unique_labels:
            mask = cur_true == gt_label
            masked_pred = cur_pred[mask]
            map_dict = dict()
            for masked_label in masked_pred:
                if masked_label in map_dict.keys():
                    map_dict[masked_label] += 1
                else:
                    map_dict.update({masked_label: 1})
            max_v = 0
            max_k = -1
            for k, v in map_dict.items():
                if v > max_v:
                    max_v = v
                    max_k = k
            new_pred[new_pred == -max_k] = gt_label
...
def clustering_acc(y_true, y_pred, num_points, dummy_class_id=0):
    ...
    for point_id in range(0, y_pred.shape[0], num_points):
        ...
        for i in range(cur_pred.size):
            cost[cur_pred[i], cur_true[i]] += 1
        # cost = cost + np.identity(D) * cost.max() * 2
        matched_id = linear_assignment(cost.max() - cost)
        new_pred = -cur_pred
        for i, j in matched_id:
            new_pred[new_pred == -i] = j
