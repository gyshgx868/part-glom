...
from part_seg_ste.models.net import PartGLOM
from part_seg_ste.runners.runner import Runner
...
class SampleRunner(Runner):
    ...
    def _eval(self):
        ...
        for batch_id, data in enumerate(self.dataset['test']):
            ...
            mid_pred_probs = []
            top_pred_probs = []
            sampler = OneHotCategorical(logits=mid_logits)
            for s in range(self.args.num_samples):
                current_probs = sampler.sample()
        ...
        with open('./sampled_results_ste_{}.txt'.format(self.args.category), 'a+') as f:
